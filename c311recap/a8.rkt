...
(define fib-cps
  (lambda (n k)    
    (cond
      ...
      [else
       (fib-cps (sub1 n) (lambda (fibsub1n)
          (fib-cps (sub1 (sub1 n))) (lambda (fibsub2n)             
             (k (+ fibsub1n fibsub2n)))))])))

(define fib-reg-driver
  (lambda (n jump)
    (define n* 'dummy)
    (define k* 'dummy)
    (define v* 'dummy)
    ...
    (define (apply-k)
      (match k*
        [`(empty-k ,jump) (begin (set! done* #t) (jump v*))]
        [`(else-k-outer ,n ,k)
         (begin
           (set! k* `(else-k-inner ,v* ,k))
           (set! n* (sub1 (sub1 n)))
           (fib-reg))]
        [`(else-k-inner ,v ,k)
         (begin
